...
BEGIN
  ...
  /*
   * Movimientos nuevos:
   * id_pedido_trazabilidad identifica directamente al pedido.
   *
   * Movimientos legacy:
   * id_ref representa id_detalle_pedido, no necesariamente id_pedido.
   */
  IF EXISTS (
    SELECT 1
    FROM public.movimientos_inventario mi
    WHERE mi.id_pedido_trazabilidad = p_id_pedido
      AND upper(btrim(COALESCE(mi.ref_origen, '')))
          IN ('PEDIDO', 'FALTANTE_COCINA')
  ) OR EXISTS (
    SELECT 1
    FROM public.detalle_pedido dp
    INNER JOIN public.movimientos_inventario mi
      ON mi.id_ref = dp.id_detalle_pedido
     AND mi.id_pedido_trazabilidad IS NULL
  ...
  LOOP
    ...
    LOOP
      ...
      ELSIF v_origen = 'EXTRA' THEN
        ...
        ELSE
          RAISE EXCEPTION USING
            ERRCODE = 'P0001', MESSAGE = 'POS_RPC_ITEM_INVALIDO',
            DETAIL = format('line_ref=%s; tipo_item=%s', v_line_ref, v_item_tipo);
        END IF;
  ...
  LOOP
     ...
     * Regla POS:
     *
     * Las ventas y pedidos no deben bloquearse por stock insuficiente
     * ni por haber alcanzado el stock minimo.
     *
     * El movimiento se registra con ref_origen='PEDIDO' y el trigger
     * fn_mov_inv_apply_stock permite que el saldo quede negativo para
     * PEDIDO y FALTANTE_COCINA.
     *
     * Se conservan todas las validaciones de existencia, almacen,
     * trazabilidad, snapshots, unidades y consistencia de consumos.
     */
...
COMMENT ON TABLE public.menu_extra_receta IS
'Relacion opcional para recomendacion y orden visual de extras por receta. No restringe la venta.';

COMMENT ON COLUMN public.menu_extra_receta.orden IS
'Orden visual opcional de recomendacion de extras por receta. No restringe la venta.';

COMMENT ON COLUMN public.menu_extra_receta.estado IS
'Estado de la recomendacion visual extra-receta. No restringe la venta.';
