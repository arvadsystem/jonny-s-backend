...
============================================================
21. TRAZABILIDAD DE ORIGEN EN DETALLE_FACTURAS
============================================================

Toda linea de public.detalle_facturas debe conservar trazabilidad del origen operativo de la venta.

Campos de trazabilidad:
- id_detalle_pedido: vinculo exacto a la linea origen en detalle_pedido.
- tipo_item: tipo historico de la linea (PRODUCTO, RECETA, COMBO, MIXTO, ITEM).
- id_receta y id_combo: referencias directas cuando aplique.
- origen_snapshot (JSONB): foto historica del origen al momento de facturar.

Reglas:
1. La factura no se reescribe despues de creada.
2. origen_snapshot guarda contexto de auditoria (tipo, nombre mostrado, ids relevantes, cantidad, precio unitario y total de linea).
3. Ventas historicas pueden quedar con tipo_item NULL; deben seguir funcionando con comportamiento legacy conservador.
4. En reversiÃ³n, solo PRODUCTO con id_producto claro puede devolver inventario de producto terminado.
5. RECETA y COMBO no devuelven insumos automaticamente.
6. MIXTO no devuelve insumos automaticamente; se mantiene regla conservadora.

Objetivo:
- Mejorar reversiÃ³n, auditoria, conciliaciÃ³n de caja/inventario y reportabilidad sin romper ventas historicas.


============================================================
22. NORMALIZACION DE ORIGEN DE LINEAS FACTURADAS
============================================================

Para separar responsabilidades:
- public.detalle_facturas conserva informacion contable/comercial de la linea facturada.
- public.detalle_facturas_origen conserva trazabilidad tecnica del origen de la linea.

Relacion:
- 1:1 por id_detalle_factura (UNIQUE en detalle_facturas_origen.id_detalle_factura).

Compatibilidad:
1. Ventas historicas pueden no tener fila en detalle_facturas_origen.
2. El backend debe tener fallback conservador a columnas legacy de detalle_facturas cuando no exista origen normalizado.
3. Columnas de trazabilidad en detalle_facturas (id_detalle_pedido, tipo_item, id_receta, id_combo, origen_snapshot) se mantienen temporalmente como deprecated para compatibilidad.

Inventario en reversiÃ³n:
1. PRODUCTO puede devolver producto terminado.
2. RECETA no devuelve insumos automaticamente.
3. COMBO no devuelve insumos automaticamente.
4. ITEM/null mantiene comportamiento conservador.

Regla sobre MIXTO:
- MIXTO se modela como combinacion de lineas PRODUCTO/RECETA/COMBO.
- No se recomienda MIXTO como tipo_item de una sola linea, salvo que exista un caso real documentado.
