...
const validateInsumoForPresentation = async (
  client,
  idInsumo,
  { requireActive = false, findInsumo = findInsumoById } = {}
) => {
  const insumo = await findInsumo(client, idInsumo);
...
const assertCompatibleBaseUnit = (insumo, presentation, message) => {
  if (Number(insumo.id_unidad_medida) === Number(presentation.id_unidad_base)) return;
  throw createHttpError(
    409,
    message || 'La unidad base de la presentacion no coincide con la unidad base actual del insumo. Corrige la presentacion antes de guardar.',
...
const validatePresentationData = async (client, idInsumo, data, { requireActiveInsumo = false } = {}) => {
  ...
  assertCompatibleBaseUnit(insumo, data);
...
export const changeInsumoPresentacionEstado = async (idInsumo, idPresentacion, estado, overrides = {}) => {
  const dependencies = {
    getClient,
    findInsumoById,
    findPresentacionById,
  ...
  try {
    const current = await dependencies.findPresentacionById(client, idPresentacion);
    if (!current) throw createHttpError(404, 'Presentacion no encontrada.', 'PRESENTACION_NOT_FOUND');
    if (Number(current.id_insumo) !== Number(idInsumo)) {
      throw createHttpError(409, 'La presentacion no pertenece al insumo indicado.', 'PRESENTACION_INSUMO_MISMATCH');
    }
    ...
    if (estado) {
      const insumo = await validateInsumoForPresentation(client, idInsumo, {
        requireActive: true,
        findInsumo: dependencies.findInsumoById
      });
      assertCompatibleBaseUnit(
        insumo,
        current,
        'No se puede activar la presentacion porque su unidad base no coincide con la unidad base actual del insumo. Corrige la presentacion antes de activarla.'
      );
    }
    await client.query('BEGIN');
    if (estado && current.uso_compra && current.es_predeterminada_compra) {
      await dependencies.clearDefaultCompra(client, idInsumo, idPresentacion);
    }
    if (estado && current.uso_receta && current.es_predeterminada_receta) {
      await dependencies.clearDefaultReceta(client, idInsumo, idPresentacion);
    }
    await dependencies.updatePresentacionEstado(client, idPresentacion, estado);
    await client.query('COMMIT');
    return { id_presentacion: idPresentacion };
  } catch (err) {
    await client.query('ROLLBACK');
